...
问题四继承问题三的双阈值接口，在$v=0.5,1,1.25,1.5,1.75,2,5,10\ \mathrm{K/min}$共八个扫描速率下，分别匹配$15\,^{\circ}\mathrm C$和$10\,^{\circ}\mathrm C$两个阈值口径，合计16个场景独立进行反求。原始厚度下的负重上限均为$t_W^{(4)}=784.916\ \mathrm{s}$，所有场景均通过可行性检查。求解结果汇总于表\ref{tab:q4-results}。
...
第二，同一扫描速率下，$10\,^{\circ}\mathrm C$阈值所需提高比例高于$15\,^{\circ}\mathrm C$阈值。例如$v=5\ \mathrm{K/min}$时，$15\,^{\circ}\mathrm C$阈值仅需提高$86.328\%$，而$10\,^{\circ}\mathrm C$阈值需提高$108.984\%$。这是因为更低的阈值要求人体表面温度在更长时间内保持在更低水平，对放热能力提出了更高要求。
...
所有场景的二分搜索区间宽度均为$6.104\times10^{-5}$，对应的百分比区间宽度均小于$0.01$个百分点，数值精度满足要求。单调性验证结果表明，在二分下界与上界处，热安全时间$t_T^{(4)}(\alpha)$随$\alpha$严格单调递增，未发现平台或异常反向变化。图\ref{fig:q4-demo}展示了$v=2,5,10\ \mathrm{K/min}$三个典型速率下的最小提高比例对比及相应有效比热曲线。

\begin{figure}[!htbp]
\centering
\includegraphics[width=\FigureDoubleWidth]{../modules/50_q4/figures/第四问演示图.png}
\caption{问题四典型扫描速率下的最小提高比例与有效比热曲线。左图对比$v=2,5,10\ \mathrm{K/min}$的$15\,^{\circ}\mathrm C$阈值最小提高比例；右图给出相应最优倍率下的有效比热曲线，标出主相变区间与峰温。}
\label{fig:q4-demo}
\end{figure}

需要指出的是，问题三的目标阈值和扫描速率可信域尚未最终锁定，上述结果对应于$15\,^{\circ}\mathrm C$和$10\,^{\circ}\mathrm C$双阈值阶段口径下的诊断结果。若问题三最终目标定义调整，问题四需严格继承同一目标定义重新反求。