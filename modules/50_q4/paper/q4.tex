...
\noindent{\small \textbf{[PAPER SYNC: STALE]}} 本问模型（Q4-M1 v1.2）已通过评审（MODEL=APPROVED），但正式结果尚未冻结（RESULT=DRAFT，NEEDS\_REVIEW）。以下给出的数值为继承问题三双阈值阶段诊断的阶段求解结果，不等同于最终答案。待问题三的目标阈值和扫描速率可信域锁定后，需严格继承同一目标定义重新反求，并同步更新正文。

问题四继承问题三的双阈值接口，在$v=0.5,1,1.25,1.5,1.75,2,5,10\ \mathrm{K/min}$共八个扫描速率下，分别匹配$15\,^{\circ}\mathrm C$和$10\,^{\circ}\mathrm C$两个阈值口径，合计16个场景独立进行反求。原始厚度下的负重上限均为$t_W^{(4)}=784.916\ \mathrm{s}$，所有场景均通过可行性检查。阶段求解结果汇总于表\ref{tab:q4-results}。

\begin{table}[!htbp]
\centering
\caption{问题四各扫描速率与阈值组合下最小放热能力提高比例（阶段诊断结果）}
...
第二，同一扫描速率下，$10\,^{\circ}\mathrm C$阈值所需提高比例高于$15\,^{\circ}\mathrm C$阈值。例如$v=5\ \mathrm{K/min}$时，$15\,^{\circ}\mathrm C$阈值阶段结果为$86.328\%$，而$10\,^{\circ}\mathrm C$阈值为$108.984\%$。这是因为更低的阈值要求人体表面温度在更长时间内保持在更低水平，对放热能力提出了更高要求。
