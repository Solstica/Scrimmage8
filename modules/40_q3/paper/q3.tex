...
实验者能够实际持续站立的时间必须同时满足热安全条件和负重条件，因此定义方案$n$的有效持续时间
\begin{equation}
\boxed{
...
&n\in\{0,1,2,3\},\\
&C_0+n\Delta C\le1.5C_0,\\
&m_h+m_c(n)\le100-0.05t_{\rm eff}(n).
\end{aligned}
}
\label{eq:q3-optimization}
\end{equation}
由于候选方案只有四个，采用完全枚举即可获得严格的全局最优方案：依次令$n=0,1,2,3$调用问题一正演求解器，计算$t_T(n)$、$t_W(n)$与$t_{\rm eff}(n)$，最后选择
\begin{equation}
...
从模型结构上看，预计$t_T(n)$随隔热厚度增加而上升，而$t_W(n)$随$n$严格下降，因此最优方案通常出现在两类限制接近的位置或费用允许的边界处。该趋势只作为机理判断，最终选择仍以四个候选方案的完整瞬态计算结果为准。

\subsection{结果输出与检验}

数值求解后应给出四个候选方案的最外层厚度、服装质量、总费用、热安全时间$t_T$、负重时间$t_W$和有效时间$t_{\rm eff}$，并标出最优$n^*$。同时检查最优方案是否由热安全约束、负重约束或预算约束决定。问题一已经采用时间步和模态截断收敛检验，本问对四个候选厚度沿用相同标准，避免厚度变化造成数值误差差异。