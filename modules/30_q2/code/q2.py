...
def basis3(M, nq):
    lam = roots(d[2], k[2], h_e, M)
    y, qw = gauss(d[2], nq)
    phi = np.sin(lam[:, None] * y)
    ends = (np.zeros(M), np.sin(lam * d[2]), lam, lam * np.cos(lam * d[2]))
...
def w3(y, T23):
    slope = h_e * (T_inf - T23) / (k[2] + h_e * d[2])
...
def outer_surface(state, b3):
    return float(w3(d[2], state["T23"]) + state["b3"] @ b3["phi_r"])
...
def modal_step(state, dt, b1, b2, b3, T_dsc, p_dsc, xi_dsc, area, L_pcm):
    ...
    gamma2 = k[1] * b2["lam"] ** 2 / (rho[1] * c_eff)
...
def snapshot(state, b1, b2, b3, scan, L_pcm):
    return [
        scan,
        L_pcm / 1000.0,
        state["t"],
        ...
        layer2_avg(state, b2),
        state["T23"],
        outer_surface(state, b3),
        state["xi"],
    ]


def profile(state, b1, b2, b3):
    rows = []
    ...
    vals = [
        ...
        w3(ys[2], state["T23"]) + eval_sin(b3, state["b3"], ys[2]),
    ]
...
def solve_case(scan, dt, M, nq, T_dsc, p_dsc, xi_dsc, area, save_detail=False):
    L_pcm = area * 60.0 / scan
    b1 = basis1(M, nq)
    b2 = basis2(M, nq)
    b3 = basis3(M, nq)
    b3_initial = project(np.full(nq, T_b) - w3(b3["y"], T_b), b3)
    state = {
        "t": 0.0,
        "T12": T_b,
        "T23": T_b,
    ...
    history = [snapshot(state, b1, b2, b3, scan, L_pcm)]
    profiles = profile(state, b1, b2, b3) if save_detail else []
    t15 = None
    t10 = None
    max_cond = 0.0
    max_res = 0.0
    ...
    while state["t"] < 1800.0 and t10 is None:
        ...
        state, cond, res = modal_step(
            state, dt, b1, b2, b3, T_dsc, p_dsc, xi_dsc, area, L_pcm
        )
        ...
        history.append(snapshot(state, b1, b2, b3, scan, L_pcm))
        if save_detail and state["t"] + 1e-10 >= next_profile:
            profiles.extend(profile(state, b1, b2, b3))
            next_profile += 10.0

    if t15 is None or t10 is None:
        raise RuntimeError(f"scan={scan:g} K/min did not reach both thresholds")
    if save_detail:
        profiles.extend(profile(state, b1, b2, b3))
    return [scan, L_pcm / 1000.0, t15, t10, max_cond, max_res, state["xi"]], history, profiles
...
def main():
    ...
    out = Path(sys.argv[1]) if len(sys.argv) > 1 else root / "modules" / "30_q2" / "results" / "formal"
    ...
    for scan in [2.0, 5.0, 10.0]:
        ans, rows, prof = solve_case(
            scan, 0.25, 40, 160, T_dsc, p_dsc, xi_dsc, area, save_detail=(scan == 5.0)
        )
        results.append(ans)
        history.extend(rows)
    ...
    for row in results:
        print(f"scan={row[0]:g} K/min, L={row[1]:.3f} kJ/kg, t15={row[2]:.3f} s, t10={row[3]:.3f} s")
